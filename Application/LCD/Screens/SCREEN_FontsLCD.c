...
int KeyboardTypeDisplay(KEYBOARD_TYPES type, SELECT_PRESS_BLOCK selBlockPress, figureShape shape, uint8_t bold, uint16_t x, uint16_t y, uint16_t widthKey, uint16_t heightKey, uint8_t interSpace, uint16_t forTouchIdx, uint16_t startTouchIdx, uint8_t eraseOther)
{
	...
	void _Key(XY_Touch_Struct pos){
		LCD_ShapeWindow( s[k].shape, 0, widthAll,heightAll, pos.x,pos.y, s[k].widthKey,s[k].heightKey, SetColorBoldFrame(frameColor,s[k].bold),fillColor,bkColor);
	}
	...
	void _KeyPress(XY_Touch_Struct pos){
		LCD_ShapeWindow( s[k].shape, 0, widthAll,heightAll, pos.x,pos.y, s[k].widthKey,s[k].heightKey, SetColorBoldFrame(framePressColor,s[k].bold),fillPressColor,bkColor);
	}
	...
	void _ServiceCoeff(void)
	{
		...
		switch((int)selBlockPress)
		{
			case KEY_All_release:
				...
				fillColor = BrightIncr(fillColor_c,0xE);  // z typu _copy dac duzo wyzej aby juz to nie deklarowac albo przez funkcji np  _Copy(param); ReCopy(param !!!!!)
				for(int i=0; i<countKey; ++i){
					i<countKey-1 ? _KeyStr(posKey[i],txtKey[i],colorTxtKey[i]) : _KeyStrDisp(posKey[i],txtKey[i],colorTxtKey[i]);
				}
				fillColor = fillColor_c;
				break;
...
static void FRAMES_GROUP_separat(int argNmb, int startOffsX,int startOffsY, int offsX,int offsY,  int boldFrame)		/* Parameters ..Offs.. is counted from STR (not from FRAME) */
{
	...
	#define _Rectan LCD_Shape(field.x-fontsFrameSpace, field.y-fontsFrameSpace, LCD_RoundRectangle, field.width+2*fontsFrameSpace, field.height+2*fontsFrameSpace, SetColorBoldFrame(_FRAME_COLOR,bold), _FILL_COLOR, v.COLOR_FillMainFrame)
...
//		LCDSHAPE_Arrow(0,SETSHAPE_Arrow(LCD_X,LCD_Y,  0,231,   SHIFT_LEFT(**(argVal+5),0,16),  SHIFT_LEFT(**(argVal+4),0,16), v.COLOR_MainFrame,YELLOW,v.COLOR_BkScreen, Right));
//		LCDSHAPE_Arrow(0,SETSHAPE_Arrow(LCD_X,LCD_Y,  150,231, SHIFT_LEFT(**(argVal+5),1,16),  SHIFT_LEFT(**(argVal+4),1,16), v.COLOR_MainFrame,DARKYELLOW,v.COLOR_BkScreen, Right));
//		LCDSHAPE_Arrow(0,SETSHAPE_Arrow(LCD_X,LCD_Y,  300,231, SetLineBold(**(argVal+5),2),  SetTriangHeightCoeff(**(argVal+4),2), v.COLOR_MainFrame,WHITE,v.COLOR_BkScreen, Right));
//		LCDSHAPE_Arrow(0,SETSHAPE_Arrow(LCD_X,LCD_Y,  450,231, SHIFT_LEFT(**(argVal+5),3,16),  SHIFT_LEFT(**(argVal+4),3,16), v.COLOR_MainFrame,BROWN,v.COLOR_BkScreen, Right));
//		LCDSHAPE_Arrow(0,SETSHAPE_Arrow(LCD_X,LCD_Y,  600,231, SHIFT_LEFT(**(argVal+5),4,16),  SHIFT_LEFT(**(argVal+4),4,16), v.COLOR_MainFrame,ORANGE,v.COLOR_BkScreen, Right));


//
//		LCDSHAPE_Window(LCDSHAPE_Arrow,0,SETSHAPE_Arrow(LCD_X,LCD_Y,  0,231,   SHIFT_LEFT(**(argVal+5),0,16),  SHIFT_LEFT(**(argVal+4),0,16), v.COLOR_MainFrame,YELLOW,v.COLOR_BkScreen, Left));
//		LCDSHAPE_Window(LCDSHAPE_Arrow,0,SETSHAPE_Arrow(LCD_X,LCD_Y,  150,231, SHIFT_LEFT(**(argVal+5),1,16),  SHIFT_LEFT(**(argVal+4),1,16), v.COLOR_MainFrame,DARKYELLOW,v.COLOR_BkScreen, Up));
//		LCDSHAPE_Window(LCDSHAPE_Arrow,0,SETSHAPE_Arrow(LCD_X,LCD_Y,  300,231, SetLineBold(**(argVal+5),2),  SetTriangHeightCoeff(**(argVal+4),2), v.COLOR_MainFrame,WHITE,v.COLOR_BkScreen, Down));
//		LCDSHAPE_Window(LCDSHAPE_Arrow,0,SETSHAPE_Arrow(LCD_X,LCD_Y,  450,231, SHIFT_LEFT(**(argVal+5),3,16),  SHIFT_LEFT(**(argVal+4),3,16), v.COLOR_MainFrame,BROWN,v.COLOR_BkScreen, Right));
//		LCDSHAPE_Window(LCDSHAPE_Arrow,0,SETSHAPE_Arrow(LCD_X,LCD_Y,  600,231, SHIFT_LEFT(**(argVal+5),4,16),  SHIFT_LEFT(**(argVal+4),4,16), v.COLOR_MainFrame,ORANGE,v.COLOR_BkScreen, Right));
	}
	else
	{
		LCD_Arrow(0,LCD_X,LCD_Y,  450,231, SHIFT_LEFT(40,4,16),  SHIFT_LEFT(100,3,16), v.COLOR_MainFrame,BROWN,v.COLOR_BkScreen, Up);

		LCD_Arrow(0,LCD_X,LCD_Y,  150,231, SHIFT_LEFT(40,4,16),  SHIFT_LEFT(100,3,16), v.COLOR_MainFrame,BROWN,v.COLOR_BkScreen, Down);
	}
...
//	lenStr=LCD_StrDependOnColorsVar(STR_FONT_PARAM(CPUusage,FillMainFrame),450, 0, 	TXT_CPU_USAGE,	 		 halfHight,0,255,ConstWidth);
	 ...
	 StartMeasureTime_us();
	 if(Test.type)
		 lenStr=LCD_StrVar(v.FONT_VAR_Fonts,v.FONT_ID_Fonts, LCD_Xmiddle(0,GetPos,v.FONT_ID_Fonts,Test.txt,Test.spaceBetweenFonts,Test.constWidth), LCD_Ymiddle(0,GetPos,v.FONT_ID_Fonts), Test.txt, fullHight, Test.spaceBetweenFonts, argNmb==0 ? v.COLOR_BkScreen : LCD_GetStrVar_bkColor(v.FONT_VAR_Fonts),  0,Test.constWidth,v.COLOR_BkScreen);
	 else
